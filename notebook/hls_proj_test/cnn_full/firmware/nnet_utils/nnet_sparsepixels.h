...
template <typename T, int INDEX_BITS>
...
template <class T, int N, class Op, class t>
T find_active(T *x, Op op, t threshold) {
#pragma HLS INLINE
    ...
    return op(find_active<T, leftN, Op, t>(x, op, threshold),
              find_active<T, rightN, Op, t>(x + leftN, op, threshold), threshold);
}

template <class data_T, class res_T, class hash_T, int N_h, int N_w, int N_c, int N_sparse>
void sparse_input_reduce(data_T input_arr[N_h * N_w * N_c], data_T threshold,
                         res_T sparse_arr_feat[N_sparse * N_c], hash_T sparse_arr_hash[N_sparse * 2]) {
...
template <class data_T, class res_T, class hash_T, class w_T, class b_T, class accum_T, int N_sparse, int n_chan,
          int n_filt, int ker_size>
...
void sparse_pooling_avg(data_T sparse_arr_feat_in[N_sparse * n_chan], res_T sparse_arr_feat_out[N_sparse * n_chan],
    ...
    // pool_size is a template parameter; compiler evaluates at compile time.
    // Preserve original numerical behavior: 1/pool_size quantized to ap_fixed<10, 0>,
    // applied twice (once per spatial dim) in the accumulation below.
