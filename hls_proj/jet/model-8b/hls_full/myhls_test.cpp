...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
      ...
      hls::stream<input_t> x_in("x_in");
      nnet::copy_data<float, input_t, 0, N_INPUT_1_1*N_INPUT_2_1*N_INPUT_3_1>(in, x_in);
      hls::stream<result_t> layer15_out("layer15_out");

            // hls-fpga-machine-learning insert top-level-function
            myhls(x_in,layer15_out);
            ...
            if (e % CHECKPOINT == 0) {
                ...
                nnet::print_result<result_t, N_LAYER_13>(layer15_out, std::cout, true);
            }
            e++;

            // hls-fpga-machine-learning insert tb-output
            nnet::print_result<result_t, N_LAYER_13>(layer15_out, fout);
        }
        fin.close();
        fpr.close();
    } else {
