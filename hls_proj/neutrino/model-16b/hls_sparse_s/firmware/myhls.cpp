...
void myhls(
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<weight5_t, 147>(w5, "w5.txt");
        nnet::load_weights_from_txt<bias5_t, 3>(b5, "b5.txt");
        nnet::load_weights_from_txt<weight10_t, 3888>(w10, "w10.txt");
    ...
    conv_default_t sparse_arr_feat_conv2_out[N_MAX_PIXELS * 3];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_conv2_out complete dim=0
    sparse_conv<model_default_t, conv_default_t, ap_uint<10>, weight5_t, bias5_t, N_MAX_PIXELS, 1, 3, 7>(sparse_arr_feat_act1_out, sparse_arr_feat_conv2_out, sparse_arr_hash_reduce_out, w5, b5); // sparse conv2
    ...
    sparse_relu<conv_default_t, model_default_t, N_MAX_PIXELS, 3>(sparse_arr_feat_conv2_out, sparse_arr_feat_act2_out); // sparse relu2
    ...
    sparse_pooling_avg<model_default_t, model_default_t, ap_uint<10>, N_MAX_PIXELS, 3, 7>(sparse_arr_feat_act2_out, sparse_arr_feat_pool1_out, sparse_arr_hash_reduce_out, sparse_arr_hash_pool1_out); // sparse pool2

    model_default_t flatten_out[9 * 9 * 3];
    #pragma HLS ARRAY_PARTITION variable=flatten_out complete dim=0
    sparse_flatten<model_default_t, ap_uint<10>, 9, 9, 3, N_MAX_PIXELS>(sparse_arr_feat_pool1_out, sparse_arr_hash_pool1_out, flatten_out); // sparse flatten

    //auto& layer9_out = layer8_out;
    dense1_result_t layer10_out[N_LAYER_10];
    #pragma HLS ARRAY_PARTITION variable=layer10_out complete dim=0
    nnet::dense<model_default_t, dense1_result_t, config10>(flatten_out, layer10_out, w10, b10); // dense1
    ...
    nnet::sigmoid<dense2_result_t, result_t, sigmoid_config15>(layer13_out, layer15_out); // sigmoid
