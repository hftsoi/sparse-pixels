...
void myhls(
    ...
    if (!loaded_weights) {
        nnet::load_weights_from_txt<weight2_t, 49>(w2, "w2.txt");
        nnet::load_weights_from_txt<bias2_t, 1>(b2, "b2.txt");
        nnet::load_weights_from_txt<weight5_t, 49>(w5, "w5.txt");
        nnet::load_weights_from_txt<bias5_t, 1>(b5, "b5.txt");
        nnet::load_weights_from_txt<weight8_t, 196>(w8, "w8.txt");
        ...
        nnet::load_weights_from_txt<weight16_t, 12>(w16, "w16.txt");
    ...
    conv_default_t sparse_arr_feat_conv1_out[N_MAX_PIXELS * 1];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_conv1_out complete dim=0
    sparse_conv<input_t, conv_default_t, ap_uint<10>, weight2_t, bias2_t, N_MAX_PIXELS, 1, 1, 7>(sparse_arr_feat_reduce_out, sparse_arr_feat_conv1_out, sparse_arr_hash_reduce_out, w2, b2); // sparse conv1
    ...
    sparse_relu<conv_default_t, model_default_t, N_MAX_PIXELS, 1>(sparse_arr_feat_conv1_out, sparse_arr_feat_act1_out); // sparse relu1

    conv_default_t sparse_arr_feat_conv2_out[N_MAX_PIXELS * 1];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_conv2_out complete dim=0
    sparse_conv<model_default_t, conv_default_t, ap_uint<10>, weight5_t, bias5_t, N_MAX_PIXELS, 1, 1, 7>(sparse_arr_feat_act1_out, sparse_arr_feat_conv2_out, sparse_arr_hash_reduce_out, w5, b5); // sparse conv2
    ...
    sparse_relu<conv_default_t, model_default_t, N_MAX_PIXELS, 1>(sparse_arr_feat_conv2_out, sparse_arr_feat_act2_out); // sparse relu2

    conv_default_t sparse_arr_feat_conv3_out[N_MAX_PIXELS * 4];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_conv3_out complete dim=0
    sparse_conv<model_default_t, conv_default_t, ap_uint<10>, weight8_t, bias8_t, N_MAX_PIXELS, 1, 4, 7>(sparse_arr_feat_act2_out, sparse_arr_feat_conv3_out, sparse_arr_hash_reduce_out, w8, b8); // sparse conv3

    model_default_t sparse_arr_feat_act3_out[N_MAX_PIXELS * 4];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_act3_out complete dim=0
    sparse_relu<conv_default_t, model_default_t, N_MAX_PIXELS, 4>(sparse_arr_feat_conv3_out, sparse_arr_feat_act3_out); // sparse relu3

    model_default_t sparse_arr_feat_pool1_out[N_MAX_PIXELS * 4];
    ...
    sparse_pooling_avg<model_default_t, model_default_t, ap_uint<10>, N_MAX_PIXELS, 4, 8>(sparse_arr_feat_act3_out, sparse_arr_feat_pool1_out, sparse_arr_hash_reduce_out, sparse_arr_hash_pool1_out); // sparse pool1

    model_default_t flatten_out[8 * 8 * 4];
    #pragma HLS ARRAY_PARTITION variable=flatten_out complete dim=0
    sparse_flatten<model_default_t, ap_uint<10>, 8, 8, 4, N_MAX_PIXELS>(sparse_arr_feat_pool1_out, sparse_arr_hash_pool1_out, flatten_out); // sparse flatten

    //auto& layer12_out = layer11_out;
    dense1_result_t layer13_out[N_LAYER_13];
    #pragma HLS ARRAY_PARTITION variable=layer13_out complete dim=0
    nnet::dense<model_default_t, dense1_result_t, config13>(flatten_out, layer13_out, w13, b13); // dense1
    ...
    nnet::sigmoid<dense2_result_t, result_t, sigmoid_config18>(layer16_out, layer18_out); // sigmoid
