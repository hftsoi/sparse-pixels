...
void myhls(
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<weight6_t, 75>(w6, "w6.txt");
        nnet::load_weights_from_txt<bias6_t, 3>(b6, "b6.txt");
        nnet::load_weights_from_txt<weight11_t, 3888>(w11, "w11.txt");
        nnet::load_weights_from_txt<bias11_t, 36>(b11, "b11.txt");
        nnet::load_weights_from_txt<weight14_t, 360>(w14, "w14.txt");
    ...
    sparse_pooling_avg<model_default_t, model_default_t, ap_uint<10>, N_MAX_PIXELS, 1, 4>(sparse_arr_feat_act1_out, sparse_arr_feat_pool1_out, sparse_arr_hash_reduce_out, sparse_arr_hash_pool1_out); // sparse pool1
    ...
    sparse_conv<model_default_t, conv_default_t, ap_uint<10>, weight6_t, bias6_t, N_MAX_PIXELS, 1, 3, 5>(sparse_arr_feat_pool1_out, sparse_arr_feat_conv2_out, sparse_arr_hash_pool1_out, w6, b6); // sparse conv2
    ...
    model_default_t flatten_out[6 * 6 * 3];
    #pragma HLS ARRAY_PARTITION variable=flatten_out complete dim=0
    sparse_flatten<model_default_t, ap_uint<10>, 6, 6, 3, N_MAX_PIXELS>(sparse_arr_feat_pool2_out, sparse_arr_hash_pool2_out, flatten_out); // sparse flatten

    //auto& layer10_out = layer9_out;
    dense1_result_t layer11_out[N_LAYER_11];
    #pragma HLS ARRAY_PARTITION variable=layer11_out complete dim=0
    nnet::dense<model_default_t, dense1_result_t, config11>(flatten_out, layer11_out, w11, b11); // dense1
