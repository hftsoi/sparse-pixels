...
void hls_dummy(
    ...
    sparse_conv<input_t, result_t, ap_uint<10>, weight2_t, bias2_t, N_MAX_PIXELS, 1, 2, 5>(sparse_arr_feat_reduce_out, sparse_arr_feat_conv1_out, sparse_arr_hash_reduce_out, w2, b2); // sparse conv1

    for (int i = 0; i < N_MAX_PIXELS * 2; i++) {
        #pragma HLS UNROLL
        layer2_out[i] = sparse_arr_feat_conv1_out[i];
    }
}
