...
void hls_dummy(
    ...
    result_t sparse_arr_feat_pool1_out[N_MAX_PIXELS * 2];
    ...
    sparse_pooling_avg<result_t, result_t, ap_uint<10>, N_MAX_PIXELS, 2, 2>(sparse_arr_feat_conv1_out, sparse_arr_feat_pool1_out, sparse_arr_hash_reduce_out, sparse_arr_hash_pool1_out); // sparse pool1

    for (int i = 0; i < N_MAX_PIXELS * 2; i++) {
        #pragma HLS UNROLL
        layer2_out[i] = sparse_arr_feat_pool1_out[i];
    }
}
