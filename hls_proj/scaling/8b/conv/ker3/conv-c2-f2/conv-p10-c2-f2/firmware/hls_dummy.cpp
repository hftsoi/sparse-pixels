...
void hls_dummy(
    ...
    if (!loaded_weights) {
        ...
        nnet::load_weights_from_txt<weight4_t, 36>(w4, "w4.txt");
        nnet::load_weights_from_txt<bias4_t, 2>(b4, "b4.txt");
        loaded_weights = true;    }
#endif
    // ****************************************
    ...
    result_t sparse_arr_feat_conv2_out[N_MAX_PIXELS * 2];
    #pragma HLS ARRAY_PARTITION variable=sparse_arr_feat_conv2_out complete dim=0
    sparse_conv<result_t, result_t, ap_uint<10>, weight4_t, bias4_t, N_MAX_PIXELS, 2, 2, 3>(sparse_arr_feat_conv1_out, sparse_arr_feat_conv2_out, sparse_arr_hash_reduce_out, w4, b4); // sparse conv2
